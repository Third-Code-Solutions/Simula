---
title: SIMULA Project Home
status: active
created: 2026-07-17
updated: 2026-07-29
owner: Principal program and engineering lead
classification: PROPOSED
source_of_truth: true
---

# SIMULA Project Home

## Start here

- [[PROJECT_CHARTER|Project charter]]
- [[PROJECT_STATE|Current project state]]
- [[../plans/MASTER_ROADMAP|Master roadmap]]
- [[../plans/active/002-phase-2-walking-skeleton|Active Phase 2 ExecPlan]]
- [[../plans/active/003-predikta-class-production-platform|Active production-platform ExecPlan]]
- [[../plans/completed/001-phase-1-product-and-architecture-definition|Completed Phase 1 ExecPlan]]
- [[../plans/completed/000-phase-0-evidence-and-discovery|Completed Phase 0 ExecPlan]]

## Governance

- [[EVIDENCE_LEDGER|Evidence ledger]]
- [[RISK_REGISTER|Risk register]]
- [[CHANGELOG|Changelog]]
- [[GLOSSARY|Glossary]]

## Research

- [[Research/PREDIKTA_TEARDOWN|Predikta public-evidence teardown]]
- [[Research/PHANTOMCROWD_IMPLEMENTATION_REFERENCE|PhantomCrowd implementation reference]]
- [[Research/NETOPIA_TEARDOWN|Netopia AI public-evidence teardown]]
- [[Research/COMPETITIVE_LANDSCAPE|Competitive landscape]]
- [[Research/PUBLIC_EVIDENCE_MATRIX|Public evidence matrix]]

## Product

- [[Product/PRD|Product requirements document]]
- [[Product/USER_JOURNEYS|User journeys]]
- [[Product/FEATURE_CATALOG|Feature catalog]]
- [[Product/NON_GOALS|Non-goals]]
- [[Product/ACCEPTANCE_CRITERIA|Phase 2 acceptance criteria]]
- [[Product/USER_DISCOVERY_PLAN|User discovery plan]]

## Methodology and data

- [[Methodology/METHODOLOGY_V0|Methodology v0]]
- [[Methodology/VALIDATION_FRAMEWORK|Validation framework]]
- [[Methodology/MODEL_CARD_TEMPLATE|Model card template]]
- [[Methodology/OUTPUT_TYPE_SYSTEM|Output type and uncertainty contract]]
- [[Data/DATA_STRATEGY|Data strategy]]
- [[Data/DATA_PROVENANCE_STANDARD|Data provenance standard]]
- [[Data/CANDIDATE_DATA_SOURCES|Candidate data sources]]
- [[Data/DEMO_DATA_POLICY|Demo data and admission policy]]

## Architecture and security

- [[Architecture/SYSTEM_ARCHITECTURE|System architecture]]
- [[Architecture/DATA_MODEL|Conceptual data model]]
- [[Architecture/API_ARCHITECTURE|API architecture]]
- [[Architecture/SIMULATION_PIPELINE|Simulation pipeline]]
- [[Architecture/AUTHORIZATION_MATRIX|Authorization and RLS matrix]]
- [[Architecture/RESOURCE_LIMITS|Phase 2 resource and rate limits]]
- [[Security/THREAT_MODEL|Threat model]]
- [[Security/PRIVACY_MODEL|Privacy model]]
- [[Security/CONTROL_TEST_MATRIX|Security control/test matrix]]
- [[Decisions/ADR-0001-PHASE-0-ARCHITECTURE-BOUNDARY|ADR-0001 — Phase 0 architecture boundary]]
- [[Decisions/ADR-0002-TOOLCHAIN-AND-GENERATED-CONTRACTS|ADR-0002 — Toolchain and contracts]]
- [[Decisions/ADR-0003-IDENTITY-TENANCY-AND-RLS|ADR-0003 — Identity, tenancy, RLS]]
- [[Decisions/ADR-0004-DATA-MODEL-VERSIONING-AND-LIFECYCLE|ADR-0004 — Data model and lifecycle]]
- [[Decisions/ADR-0005-API-AND-CONTRACT-COMPATIBILITY|ADR-0005 — API contracts]]
- [[Decisions/ADR-0006-QUEUE-AND-RUN-STATE-MACHINE|ADR-0006 — Queue and run state]]
- [[Decisions/ADR-0007-PROVIDER-BOUNDARY-AND-MOCK|ADR-0007 — Provider boundary]]
- [[Decisions/ADR-0008-ENVIRONMENTS-DEPLOYMENT-AND-MIGRATIONS|ADR-0008 — Environments and migrations]]
- [[Decisions/ADR-0009-OBSERVABILITY-AUDIT-AND-SERVICE-OBJECTIVES|ADR-0009 — Observability and objectives]]
- [[Decisions/ADR-0010-EXPORT-SHARE-AND-STORAGE-SEAM|ADR-0010 — Export/share deferral]]

- [[Decisions/ADR-0011-NESTJS-BULLMQ-CONTROL-PLANE|ADR-0011 - NestJS and BullMQ control plane]]
- [[Decisions/ADR-0012-PHANTOMCROWD-DERIVED-BEHAVIORAL-ENGINE|ADR-0012 - PhantomCrowd-derived behavioral engine]]
- [[Decisions/ADR-0013-GOVERNED-PGVECTOR-RETRIEVAL|ADR-0013 - Governed pgvector retrieval]]
- [[Decisions/ADR-0014-PRIVATE-STIMULUS-ASSET-PIPELINE|ADR-0014 - Private stimulus asset pipeline]]
- [[Decisions/ADR-0015-ASSET-BOUND-TECHNICAL-IMAGE-PROFILE|ADR-0015 - Asset-bound technical image profile]]

## Quality and operations

- [[QA/TEST_STRATEGY|Test strategy]]
- [[QA/EVALUATION_STRATEGY|Evaluation strategy]]
- [[QA/TRACEABILITY_MATRIX|Phase 2 traceability matrix]]
- [[QA/PHASE_2_BACKLOG|Phase 2 implementation backlog]]
- [[QA/PHASE_2_AUDIT_2026-07-18|Phase 2 audit — 2026-07-18]]
- [[Operations/DEPLOYMENT_ARCHITECTURE|Deployment architecture]]
- [[Operations/OBSERVABILITY|Observability]]
- [[Operations/INCIDENT_RESPONSE|Incident response]]
- [[Operations/RUNBOOK_RUN_CREATION_DISABLED|Run creation disabled runbook]]
- [[Operations/BACKUP_AND_RESTORE|Backup and restore]]
