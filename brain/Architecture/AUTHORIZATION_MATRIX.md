...
|---|---|---|---|---|---|---|
| `api.organizations` | — | R member org | R | R | — | C only through atomic command; U/D deferred |
...
- Tenant read through `simula_api`: `private.is_org_member(organization_id, auth.uid())`.
...
No Storage bucket or object is used in Phase 2. Future private-object policies must map `organization_id`, object record, and current membership; path prefix alone is insufficient.
