...
`OPS-RESTORE-001` creates a checksumed full PostgreSQL dump, restores it into a
separate isolated database, derives and verifies the checked-in migration head
(currently `20260730180000`), preserves and compares the source run-creation
control state, discovers every current `api` and `private` table and compares
all row counts, and removes the temporary database/artifact. It runs inside
root `pnpm verify` and CI. This proves synthetic database content
recoverability only; global roles/passwords, runtime-role ACL/Auth/API/worker
compatibility, queue/storage, and provider-managed services remain staging
drill requirements.
...
Provider-native point-in-time recovery, storage versioning, cross-region need, independent audit retention, key recovery, contractual retention, legal hold, and verified deletion propagation must be decided and drilled in Phase 5–7. Restore tests must reapply durable organization-deletion tombstones before reopening traffic, keep completed manifests minimized, and reconcile any pending request against storage/queue/cache absence before restoring tenant access.
