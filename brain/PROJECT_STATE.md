...
- OBSERVED 2026-07-18: the hosted Supabase target is `ywiwmczccktwzqyhzhiz` (Simula, active). MCP-authorized access as `kurtgav` bootstrapped the four least-privilege roles and applied checked-in migrations seed-free. Remote migration history exactly matches Git through `20260718050000`; cancellation/retry helpers, API/worker ACLs, and temporary-schema-CREATE revocations were verified (E-5011, E-5016, E-5017).
...
- M5 / P2-06 cancellation and timeout-retry sub-slices are locally and hosted-schema proven: checked-in forward migrations add an owner/editor-only cancel command, narrow RLS/event/audit authority, no-active-lease cancellation finalization, cancel-wins completion/failure transitions, cancellation-aware dispatch claiming, durable temporary-schema-CREATE cleanup, and database-authoritative 5s/30s timeout retry timing with three-attempt exhaustion. API/UI expose documented empty-JSON `POST /runs/{id}/cancel`; the worker finalizes before dispatch and defers only after a database-authorized retry resolution. Local reset/pgTAP/type checks, API/worker tests, real authorization/cancel-versus-completion/timeout-exhaustion integrations, four-browser-test P2 gate, and full `pnpm check` pass; hosted history/ACL verification also passes (E-5015–E-5017). Poison handling and lease/Redis-loss recovery are still open; P2-06 remains active.
