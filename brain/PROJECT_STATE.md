...
- OBSERVED 2026-07-18: hosted migration history equals the composed local migration set through version `20260718113445`. The lease/attempt hardening migration is applied seed-free; four affected functions retain their worker-owner/security settings and hosted security advisors report zero lints. Application relations intentionally reside in non-exposed `api` and `private` schemas, so an empty `public` Table Editor is expected (E-5022, E-5024).
...
- M6 / P2-07 is reopened. Its prior self-audit gate remains valid historical evidence (E-5022), but the independent review failed (E-5023). The expired-lease/stale-attempt High finding is remediated locally and hosted (E-5024); all remaining Critical/High findings must be cleared before re-review.
