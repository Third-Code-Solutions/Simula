...
- M5 / P2-06 cancellation, timeout-retry, stale-dispatch-recovery, poison-dispatch, and failure-experience sub-slices are locally and hosted-schema proven: checked-in forward migrations add an owner/editor-only cancel command, narrow RLS/event/audit authority, no-active-lease cancellation finalization, cancel-wins completion/failure transitions, cancellation-aware dispatch claiming, durable temporary-schema-CREATE cleanup, database-authoritative 5s/30s timeout retry timing with three-attempt exhaustion, a worker-only stale lease/Redis-loss reconciler that supersedes outbox generations under run-then-outbox locks, and a worker-only terminalizer for expired unconfirmed tenth dispatch claims. API/UI expose documented empty-JSON `POST /runs/{id}/cancel`; failed state gives explicit no-substitute-result copy and never fetches result/provenance. The worker finalizes cancellation and poison before dispatch, defers only after a database-authorized retry resolution, and periodically reconciles stale dispatches. Local reset/pgTAP/type checks, API/worker tests, real authorization/cancel-versus-completion/timeout-exhaustion/stale-lease/poison-cancel integrations, five-browser-test P2 gate, and every constituent repository quality gate pass; hosted history/ACL verification also passes where migrations apply (E-5015–E-5020). Additional safe retry classes remain open; P2-06 remains active.
