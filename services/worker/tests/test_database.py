...
import pytest
import simula_worker.database as database_module
...
class _Connection:
    def __init__(self, *, migration_version: str = "20260802150000") -> None:
...
async def test_worker_database_readiness_requires_the_exact_schema_head() -> None:
    ...
    database._migration_head = "20260802150000"
...
async def test_transaction_deadline_discards_unresponsive_connection(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    events: list[str] = []

    class HungConnection(_Connection):
        async def execute(
            self, query: str, parameters: tuple[object, ...] | None = None
        ) -> _Cursor:
    ...
    monkeypatch.setattr(database_module, "_TRANSACTION_TIMEOUT_SECONDS", 0.01)
    database = WorkerDatabase.__new__(WorkerDatabase)
    database._pool = cast(Any, CheckedPool(HungConnection()))
    database._telemetry = None
    with pytest.raises(TimeoutError):
        async with database._transaction():
            pytest.fail("timed-out setup must never yield")
    assert events == ["closed", "released"]
