...
| F16 | P2 gap   | LOCALLY VERIFIED mechanics: real API/worker preregistration, later independent source admission, actual custodian file upload, bound backtest result and refresh pass. Bound/legacy result reads reject revoked source422. Report/calibration/backtest six desktop/mobile views have zero Axe/overflow/errors.47focused Python and28Campaign Lab tests pass. | Registry approval remains separate administrator process; synthetic scoped historical comparison is not scientific validation. Final emergency-pause guard/migration, immutable gates and hosted application/schema verification remain pending. |
...
| F27 | P2       | LOCAL VERIFIED: full pinned Python3.14.7 environment; fresh migration replay441pgTAP; final-schema types bytecheck; full integration30passes before subsequent scoped fixes; current queue faults5passes and runtime faults42passes; Redis8.2.7 exact digest exercised.                                                                                      | Root final whole-repository and signed release gates remain authoritative and in progress. Subsequent scoped tests are not represented as another full-suite run.                                                                                |
| F28 | P2       | PARTIAL: current setup/project/capability/release/runbook and route evidence reconciled with verified local report/calibration/backtest mechanics, failed newer release gate, hosted controls and new Campaign Lab pause-latch rollout blocker. Historical snapshots retained.                                                                               | Update final guard/migration evidence, immutable gates and actual release receipts; legal/research acceptance and untested production roles/recovery remain explicit.                                                                            |
...
## Current rollout blocker - Campaign Lab emergency pause
